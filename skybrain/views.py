...
class SkybrainCustomerRoom(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user_type in ["visitor", "contributor"]:
...
class RoomAccessShare(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if access_type == "contributor":
            url = settings.DEPLOYED_HOST + room.organization.name + "/home/" + room.room_id + "?contributor=true"
        else:
            url = settings.DEPLOYED_HOST + room.organization.name + "/home/" + room.room_id + "?visitor=true"
        for email in emails:
